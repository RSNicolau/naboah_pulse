...
import { Upload, Link as LinkIcon, FileText, Database, Plus, RefreshCw, Check, Clock, Trash2 } from 'lucide-react';

export default function SourceManager() {
    const sources = [
        { id: '1', name: 'Reembolso_Politica_2025.pdf', type: 'file', status: 'active', date: '2 horas atrás' },
        { id: '2', name: 'https://docs.naboah.com', type: 'url', status: 'indexing', date: 'Aguardando...' },
        { id: '3', name: 'Zendesk Knowledge Base', type: 'integration', status: 'error', date: 'Falha na Sync' },
    ];
                    ...
                    <div key={s.id} className="p-6 bg-bg-0 border border-stroke rounded-3xl flex items-center justify-between group hover:border-white/20 transition-all">
                        <div className="flex items-center gap-6">
                            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center relative ${s.status === 'active' ? 'bg-success/10 text-success' :
                                ...
                                {s.type === 'integration' && <Database size={24} />}
                            ...
                            <div className="flex flex-col">
                                ...
                                <span className="text-[10px] font-medium text-text-3 uppercase tracking-widest">Sincronizado {s.date}</span>
                            </div>
                        </div>
