...
import { AlertCircle, UserMinus, ShieldAlert, ArrowRight, MessageSquare, Phone, MoreHorizontal, Loader2 } from 'lucide-react';
...
export default function ChurnRiskAlert() {
    ...
    useEffect(() => {
        apiGet<AnalyticsSummary>('/analytics/summary')
            .then((data) => {
                // Build risk entries from summary data
                const entries: RiskEntry[] = [];
                if (data.risk_score > 0) {
                    entries.push({
                        id: '1',
                        customer: `${data.customers} clientes monitorados`,
                        score: Math.round((1 - data.risk_score) * 100),
                        trend: data.risk_score > 0.3 ? 'down' : 'stable',
                        reason: `Risk score: ${data.risk_score.toFixed(2)} | ${data.threats_blocked} ameaças bloqueadas`,
                        owner: 'Pulse AI',
                    });
                }
                if (data.conversion_rate < 0.5) {
                    entries.push({
                        id: '2',
                        customer: `Conversão abaixo do target`,
                        score: Math.round(data.conversion_rate * 100),
                        trend: 'down',
                        reason: `Taxa de conversão: ${(data.conversion_rate * 100).toFixed(1)}% | ${data.deals_count} deals ativos`,
                        owner: 'Pulse AI',
                    });
                }
                setRisks(entries);
        ...
        <div className="bg-bg-1 border border-stroke rounded-[2rem] p-8 flex flex-col gap-8 shadow-2xl overflow-hidden relative group">
            ...
            {loading ? (
                ...
                <div className="flex flex-col items-center justify-center py-10 gap-3">
                    ...
                    <p className="text-sm text-text-3">Nenhum risco de churn detectado.</p>
                </div>
            ) : (
