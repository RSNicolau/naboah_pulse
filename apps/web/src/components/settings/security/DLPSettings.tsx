...
import { EyeOff, ShieldCheck, Plus, Trash2, CheckCircle2, AlertCircle, Sparkles, Database, Loader2 } from 'lucide-react';
...
export default function DLPSettings() {
    ...
    useEffect(() => {
        apiGet<DLPData>('/shield/policies')
            .then(setData)
            .catch(() => {})
        ...
        <div className="bg-bg-1 border border-stroke rounded-[2rem] p-8 flex flex-col gap-8 shadow-2xl relative group">
            ...
            {loading ? (
                ...
                <div className="flex flex-col items-center justify-center py-12 gap-3">
                    <EyeOff size={28} className="text-text-3" />
                    <p className="text-sm text-text-3">Nenhuma politica de DLP configurada.</p>
                </div>
            ) : (
            ...
            {insight ? (
                <div className="p-6 bg-surface-1 border border-dashed border-stroke rounded-2xl flex items-center gap-4">
                    ...
                    <p className="text-[11px] text-text-2 font-medium">
                        <span className="font-bold text-white uppercase tracking-tighter">Pulse Insight:</span> {insight}
                    </p>
                </div>
            ) : null}
